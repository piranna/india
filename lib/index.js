...
function diffInterface (interface1, interface2, hash1, hash2) {
  ...
  rules.forEach(function (rule) {
    ...
    try {
      ...
      breaks[rule.type].push(e.message)
    }
  })
...
function suggestVersion (version, breaks) {
  ...
  var major = breaks.major.length
  var minor = breaks.minor.length

  if (semver.lt(version, '1.0.0') && (major || minor)) {

    newVersion = semver.inc(version, 'minor')

    util.puts(chalk.bold(
      chalk.red('Found', major, 'backwards-incompatible API ' + pluralize('changes', major) + '.\n') +
      (minor ? chalk.blue('Found', minor, 'backwards-compatible API ' + pluralize('changes', minor) + '.') + '\n' : '') +
    ...
    util.puts(chalk.bold(
      chalk.blue('Found', minor, 'backwards-compatible API ' + pluralize('changes', minor) + '.\n') +
      'Recommend minor version bump',
      '(' + version + ' => ' + newVersion + ').'
    ))
